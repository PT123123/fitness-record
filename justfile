...
# Build the debug APK
build:
    set -euo pipefail; AH="$(just _detect_sdk)"; AH="${AH//\\//}"; if [ ! -d "${AH}" ]; then echo "❌ ANDROID_HOME 目录不存在: ${AH}"; exit 1; fi; echo "✅ ANDROID_HOME=${AH}"; SRC="{{source_dir()}}"; SRC="${SRC//\\//}"; cd "${SRC}"; if [ -x "./gradlew" ]; then GRADLE="./gradlew"; else GRADLE="$(command -v gradle || true)"; fi; if [ -z "${GRADLE}" ]; then echo "❌ 未找到 gradle。"; exit 1; fi; echo "🔨 构建 debug APK ..."; ${GRADLE} assembleDebug; echo ""; echo "🎉 构建成功！APK: app/build/outputs/apk/debug/app-debug.apk"
